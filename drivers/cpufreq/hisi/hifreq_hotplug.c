...
#define DEFAULT_STAY_UP_DELAY	(0 * USEC_PER_MSEC)
	...
	if (offline) {
		...
		if (!hmp_boost && !bL_cpufreq_data.boost &&
			...
			trace_hifreq_hotplug_time_notyet(cpu,
				policy->cur, target_freq, nr_runnings, bL_cpufreq_data.boost, offline);
			spin_unlock_irqrestore(&(bL_cpufreq_data.hotplug_lock), flags);
			goto set_freq;
		}

		trace_hifreq_hotplug_info(cpu,
			policy->cur, target_freq, nr_runnings, bL_cpufreq_data.boost, offline,
			bL_hifreq_max_load, bL_cpufreq_data.req_up_cnt);
		...
		trace_hifreq_hotplug_info(cpu,
			policy->cur, target_freq, nr_runnings, bL_cpufreq_data.boost, offline,
			bL_hifreq_max_load, bL_cpufreq_data.req_down_cnt);

		if (!hmp_boost && !bL_cpufreq_data.boost && (bL_hifreq_max_load * policy->cur / THRESHOLD_FREQ) > bL_cpufreq_data.single_core_load_down_threshold &&
