...
extern int ipa_get_sensor_value(u32 sensor, long *val);
...
unsigned long get_soc_temp(void)
{
	if ((IPA_INIT_OK == thermal_info.ipa_thermal[SOC].init_flag) && thermal_info.ipa_thermal[SOC].tzd) {
		if (thermal_info.ipa_thermal[SOC].tzd->temperature < 0)
			return 0;
		return (unsigned long)thermal_info.ipa_thermal[SOC].tzd->temperature; /*lint !e571*/
	}
...
static int get_temp_value(void *data, long *temp)
{
	...
	if (IPA_SENSOR_MAXID == sensor->sensor_id) {
		...
		trace_IPA_get_tsens_value(0L, 0L, 0L, val);
		if (ret)
			return ret;
		val = val < 0 ? 0 : val;
	} else {
	...
	if (!sensor->prev_temp)
	 ...
	 /*lint -e776*/
	est_temp = mul_frac((long)sensor->alpha, val) +
		mul_frac((long)(int_to_frac(1) - sensor->alpha), sensor->prev_temp);
	 /*lint +e776*/
...
static int ipa_register_soc_cdev(struct ipa_thermal *thermal_data, struct platform_device *pdev)
{
	...
	thermal_data->cdevs = kcalloc((size_t)NUM_CLUSTERS, sizeof(struct thermal_cooling_device *), GFP_KERNEL);
