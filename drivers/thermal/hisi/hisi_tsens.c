...
static int tsens_tz_degc_to_code(unsigned long degc)
{
	...
	if ((adc_start_value == 0) || (adc_end_value == 0))
		goto ERROR;

	if ((long)degc > TSENS_TEMP_END_VALUE)
		return adc_end_value;
	else if ((long)degc < TSENS_TEMP_START_VALUE)
		return TSENS_THRESHOLD_MIN_CODE;

	code = adc_start_value + (degc - TSENS_TEMP_START_VALUE) * (adc_end_value - adc_start_value) /
			(TSENS_TEMP_END_VALUE - TSENS_TEMP_START_VALUE);

	return code;

ERROR:
	pr_debug("temp_to_adc_conversion failed \n");
	return code;
}
#endif
