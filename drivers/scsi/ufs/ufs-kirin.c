				...
				struct ufs_kirin_host *host)
{
	if (host->caps & USE_HISI_MPHY_TC) {
		...
		ufshcd_dme_set(hba, UIC_ARG_MIB_SEL(0x00c1, 0x0),
			       0x1); /*RG_PLL_CP*/
		ufshcd_dme_set(hba, UIC_ARG_MIB_SEL(0x00d4, 0x0),
			       0x51); /*RG_PLL_DMY0*/
				  ...
				  struct ufs_kirin_host *host)
{
	...
	if (of_find_property(np, "ufs-kirin-use-cs-tc", NULL)) {
		host->caps |= USE_CS_TC;
	}
