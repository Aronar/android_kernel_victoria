...
static int32_t kirin_pcie_get_dtsinfo(u32 *rc_id, struct platform_device *pdev)
{
	...
	if (!np) {
		PCIE_PR_ERR("Can't find kirin_pcie node from dts");
		return -1;
	}

	if (of_property_read_u32(np, "rc_num", &g_rc_num)) {
		PCIE_PR_ERR("Can't find rc_num propery from dts");
		return -1;
	}
...
static void kirin_pcie_shutdown_prepare(struct pci_dev *dev)
{
	...
	if (index < 100)
		...
		PCIE_PR_INFO("Didn't get PME ACK ");
...
static int kirin_pcie_usr_suspend(u32 rc_idx)
{
	...
	if (rc_dev == NULL) {
		PCIE_PR_ERR("Failed to find RC dev");
		return -1;
	}
