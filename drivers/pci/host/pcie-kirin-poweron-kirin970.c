...
static int kirin_pcie_eco(struct kirin_pcie *pcie)
{
	...
	if (!sramdata) {
		PCIE_PR_ERR("ioremap sramdata fail\n");
		return -1;
	}
	...
	if (stat.size > SRAM_SIZE) {
		PCIE_PR_ERR("file size is too large");
		goto FILP_READ_FAIL;
	}

	if (dtsinfo->eco == ECO_TEST) {
		memset_io(sramdata, 0x0, 16);
	...
	pos = 0;
	len = vfs_read(fp, (char __user *)sramdata, SRAM_SIZE, &pos);
	if (len < SRAM_SIZE) {
	...
	if (dtsinfo->eco == ECO_TEST) {
		if (memcmp(sramdata, checkdata, SRAM_SIZE) == 0) {
...
int kirin_pcie_turn_on(struct pcie_port *pp)
{
	...
	if (!pmctrl) {
		PCIE_PR_ERR("pmctrl ioremap fail");
		return -1;
	}
...
int kirin_pcie_turn_off(struct pcie_port *pp)
{
	struct kirin_pcie *pcie = to_kirin_pcie(pp);//lint !e826
	void __iomem *pmctrl;
	u32 reg_val;
