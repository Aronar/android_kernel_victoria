...
struct dsm_client *dsm_client_smartpa = NULL;
#endif
...
#ifdef CONFIG_HUAWEI_DSM
	dsm_client_smartpa = dsm_register_client(&dsm_smartpa);
    if(NULL == dsm_client_smartpa) {
		dsm_client_smartpa = dsm_find_client(dsm_smartpa.name);
		if(NULL == dsm_client_smartpa) {
			dev_err(&client->dev, "dsm_audio_client register failed!\n");
		} else {
			dev_err(&client->dev, "dsm_audio_client find in dsm_server\n");
		}
	}
#endif
