...
static struct charger_dsm err_count[] = {
	{ERROR_FCP_VOL_OVER_HIGH, true, false, .dump = fcp_reg_dump, "fcp vbus is high "},
...
static void charge_start_charging(struct charge_device_info *di)
{
	...
	if(CHARGER_TYPE_NON_STANDARD == di->charger_type && di->bc_again_need){
		schedule_delayed_work(&di->bc_again_work, msecs_to_jiffies(RECHECK_INIT_TIME_INTERVAL));
	}
}
...
static void charge_stop_charging(struct charge_device_info *di)
{
	...
	if(di->bc_again_need){
		cancel_delayed_work_sync(&di->bc_again_work);
	}
...
/**********************************************************
*  Function:        charge_bc_again_work
*  Description:    recheck charger type in the product without switch-chip
*  Parameters:    work:bc_again workqueue
*  return value:   NULL
**********************************************************/
static void charge_bc_again_work(struct work_struct *work)
{
    struct charge_device_info *di =
        container_of(work, struct charge_device_info, bc_again_work.work);
    ...
    if(CHARGER_TYPE_NON_STANDARD == di->charger_type){
        hisi_usb_otg_bc_again();
        schedule_delayed_work(&di->bc_again_work, msecs_to_jiffies(RECHECK_REGULATOR_TIME_INTERVAL));
    }else{
        hwlog_info("charger type has been updated to %d already!\n", di->charger_type);
    }
}
...
static void charge_fault_work(struct work_struct *work)
{
	...
	case CHARGE_FAULT_WEAKSOURCE:
		...
		di->charge_fault = CHARGE_FAULT_NON;
		charger_dsm_report(ERROR_WATCHDOG_RESET, NULL);
		break;
	default:
		break;
	}
...
static void charge_parse_dts(struct charge_device_info *di)
{
	...
	ret = of_property_read_u32(of_find_compatible_node(NULL, NULL, "huawei,charger"), \
			"bc_again_need", &di->bc_again_need);
	if (ret) {
		hwlog_info("get bc_again_need fail!\n");
		di->bc_again_need = 0;
	}
	hwlog_info("bc_again_need = %d\n", di->bc_again_need);
...
static int charge_probe(struct platform_device *pdev)
{
	...
	if(di->bc_again_need){
		INIT_DELAYED_WORK(&di->bc_again_work, charge_bc_again_work);
	}
...
static int charge_resume(struct platform_device *pdev)
{
	...
	if (di->charger_source == POWER_SUPPLY_TYPE_MAINS) {
		...
		if(di->bc_again_need){
			schedule_delayed_work(&di->bc_again_work, msecs_to_jiffies(0));
		}
	}
