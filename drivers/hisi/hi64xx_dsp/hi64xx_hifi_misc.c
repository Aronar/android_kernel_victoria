...
static int hi64xx_ir_gpio_switch(int val)
{
	int ret = 0;
	unsigned int gpio_id = dsp_priv->dsp_config.ir_switch_gpio;

	HI64XX_DSP_INFO("ir gpio %d set %d\n", gpio_id, val);
	if (gpio_id == 0) {
		HI64XX_DSP_ERROR("gpio is invalid %d\n", gpio_id);
		ret = -1;
		return ret;
	}
	...
	if (ret < 0) {
		HI64XX_DSP_ERROR("gpio_request fail %d\n", gpio_id);
		return ret;
	}

	ret = gpio_direction_output(gpio_id, val);
	if(ret < 0){
		HI64XX_DSP_ERROR("could not set gpio:%d direction out \n",  gpio_id);
		gpio_free(gpio_id);
		return ret;
	}
...
static void hi64xx_ir_set_ocp(unsigned int value)
{
	if (dsp_priv->dsp_config.ana_rwreg_106)
		hi64xx_hifi_reg_write_bits(dsp_priv->dsp_config.ana_rwreg_106,
			value << dsp_priv->dsp_config.ana_rwreg_106_ocp, 0x3U << dsp_priv->dsp_config.ana_rwreg_106_ocp);
	else
		HI64XX_DSP_WARNING("ana_rwreg_106 is null\n");
...
// current not support HOOK
	...
	case MLIB_PATH_IR_LEARN:
		...
		ret = hi64xx_ir_gpio_switch(GPIO_PULL_DOWN);
		if (ret != OK) {
			goto end;
		}
	...
	case MLIB_PATH_IR_TRANS:
		...
		ret = hi64xx_ir_gpio_switch(GPIO_PULL_UP);
		if (ret != OK) {
			goto end;
		}

		hi64xx_ir_set_ocp(HI6403_CODEC_ANA_RWREG_106_OCP_400);

		ret = hi64xx_request_pll_resource(HI_FREQ_SCENE_IR_TRANS);
		if (ret != OK) {
			if (hi64xx_ir_gpio_switch(GPIO_PULL_DOWN))
				HI64XX_DSP_ERROR("switch ir gpio false\n");
			goto end;
		}
...
// current not support HOOK
		...
		hi64xx_ir_set_ocp(HI6403_CODEC_ANA_RWREG_106_OCP_0);
