...
/********************************************************************
*Function:	pimon_mntn_get_cur_blk
*Description:	获取pimon当前写的地址
*Input:		blk_index:pimon当前写的blk id
*Output:	NA
*Return:	pimon当前写的地址
********************************************************************/
static u64 pimon_mntn_get_last_addr(u32 blk_index)
{
	u32 reg_value;
	u64 last_addr, addr_info, blk_info;

	reg_value = readl_relaxed((u8 __iomem *)g_pimon_mgr.reg_base_addr +
		PIMON_REG_OFFSET_LAST_WADDR_LO);
	addr_info = (unsigned)((u64)(reg_value &
		PIMON_REG_BIT_MASK_LAST_WR_OFFSET) <<
		PIMON_REG_BIT_LAST_WR_OFFSET);
	blk_info = (unsigned)((u64)(g_pimon_mgr.blocks[blk_index].addr &
		PIMON_REG_BIT_MASK_LAST_WR_BASE) <<
		PIMON_REG_BIT_LAST_WR_BASE);
	last_addr = blk_info + addr_info;
	return last_addr;
}
...
static int pimon_save_data_mul_cont(char *filename)
{
	...
	ret = pimon_savedata2fs(PIMON_PATH, filename,
		blks_map_addr[last_blk_id],
		(u32)(last_addr - blks_phys_addr[last_blk_id]), 1);
	if (ret) {
		pr_err("[%s]: pimon_savedata2fs: save block%u data failed!\n",
			__func__, last_blk_id);
		return 1;
	}
	return 0;
}
