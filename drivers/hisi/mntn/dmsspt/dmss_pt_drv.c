...
static ssize_t dmsspt_finished_show(char *kbuf)
{
	...
	if(dmsspt_is_finished())
	{
		cnt = snprintf(kbuf, (ssize_t) DMSSPT_SHOW_LEN_MAX, "#finished#\n");
	}
	else
	{
		cnt = snprintf(kbuf, (ssize_t) DMSSPT_SHOW_LEN_MAX, "#saving#\n");
	}
...
static ssize_t dmsspt_region_show(char *kbuf)
{
	...
	cnt = snprintf(kbuf, (ssize_t) DMSSPT_SHOW_LEN_MAX, "base=0x%pK, size=0x%llx\n",\
...
static ssize_t dmsspt_intlv_show(char *kbuf)
{
	...
	cnt = snprintf(kbuf, (ssize_t) DMSSPT_SHOW_LEN_MAX, "intlv=%u, ch4_order=%u\n",\
