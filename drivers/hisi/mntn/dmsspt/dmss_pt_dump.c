...
int test_dmsspt_dump_trace(u64 buffer_size, u64 data_size, u64 write_velocity, u32 intlv, u32 intlv_mode, int is_sync)
{
	...
	memset(&pt_stat, 0 , sizeof(struct pattern_trace_stat));
	memset(&pt_info, 0 , sizeof(struct pattern_trace_info));
	buffer_old_size = pt_region.size;
	pt_region.size = buffer_size * 1024 * 1024;
	pt_buffer_end = (u8*)pt_region.base + pt_region.size;
	memset(g_trace_buffer_vaddr, 0xff, buffer_old_size);
	memset(g_trace_buffer_vaddr, 0x0, pt_region.size);
	...
	while (1) {
		...
		while(pt_info.pt_wptr[0] > (u64)v2p((u64)pt0)) {
			memset(pt0, poison, (u64)pt_info.intlv);
			memset(pt0 + pt_region.size/2, poison, (u64)pt_info.intlv);
			temp = (u64)pt0;
		...
		while(pt_info.pt_wptr[1] > (u64)v2p((u64)pt1)) {
			memset(pt1, poison, (u64)pt_info.intlv);
			memset(pt1 + pt_region.size/2, poison, (u64)pt_info.intlv);
			temp = (u64)pt1;
...
static struct file* dmsspt_open(int trace_num, int index)
{
	...
	pr_err("%s: enter!\n", __func__);
	if (!index) {
		memset(g_dmsspt_path[trace_num], 0, DMSSPT_PATH_MAXLEN - 1);
		g_dmsspt_path[trace_num][DMSSPT_PATH_MAXLEN - 1] = '\0';

		wait_for_completion(&dmsspt_date);
		snprintf(g_dmsspt_path[trace_num], DMSSPT_PATH_MAXLEN-1, "%s%d_%s", DMSSPT_ROOT"dmsspt", trace_num, dmsspt_get_timestamp());
		complete(&dmsspt_date);

		strncpy(path, g_dmsspt_path[trace_num], DMSSPT_PATH_MAXLEN-1);
	} else
		snprintf(path, DMSSPT_PATH_MAXLEN, "%s.%d", g_dmsspt_path[trace_num], index);
...
static void dmsspt_finish(long trace_num)
{
	...
	if (finish) {
		...
		g_pt_info.intlv = 0;
		g_pt_info.intlv_mode = DMSSPT_NOINIT_MAGIC;
		memset(&g_pt_stat, 0, sizeof(struct pattern_trace_stat));
	}
}
