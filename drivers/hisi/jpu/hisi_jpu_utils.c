...
static int hisijpu_set_crop(struct hisi_jpu_data_type *hisijd)
{
	...
	if (HISI_JPEG_DECODE_MODE_FULL_SUB >= jpu_req->decode_mode) {
		...
		/*lint -save -e737*/
		pjpu_dec_reg->crop_horizontal = jpu_set_bits32(pjpu_dec_reg->crop_horizontal,
			(((uint32_t)(jpu_req->region_info.right - 1)) << 16) | (jpu_req->region_info.left - 1), 32, 0);
		pjpu_dec_reg->crop_vertical = jpu_set_bits32(pjpu_dec_reg->crop_vertical,
			(((uint32_t)(jpu_req->region_info.bottom - 1)) << 16) | (jpu_req->region_info.top - 1), 32, 0);
		/*lint -restore*/
	}
...
int hisijpu_regulator_enable(struct hisi_jpu_data_type *hisijd)
{
	...
	inp32(hisijd->pmctrl_base + 0x384);
	udelay(1);

	inp32(hisijd->pmctrl_base + 0x388);
	udelay(1);
	...
	//module clock enable 2
	outp32(hisijd->media1_crg_base + 0x84, 0x00600060);
	outp32(hisijd->media1_crg_base + 0x010, 0x1E198000);
	udelay(1);

	//module clk disable 3
	outp32(hisijd->media1_crg_base + 0x14, 0x1E018000);
	udelay(1);

	//module iso disable 4
...
int hisijpu_clk_enable(struct hisi_jpu_data_type *hisijd)
{
...
#ifdef CONFIG_NO_USE_INTERFACE
    /* note: regulator_enable have enable axim apbm clk */
	if (hisijd->isp_axim_clk) {
		ret = clk_prepare(hisijd->isp_axim_clk);
		if (ret) {
			HISI_JPU_ERR("isp_axim_clk clk_prepare failed, error=%d!\n", ret);
			return -EINVAL;
		}

		ret = clk_enable(hisijd->isp_axim_clk);
		if (ret) {
			HISI_JPU_ERR("isp_axim_clk clk_enable failed, error=%d!\n", ret);
			return -EINVAL;
		}
	}

	if (hisijd->isp_apbm_clk) {
		ret = clk_prepare(hisijd->isp_apbm_clk);
		if (ret) {
			HISI_JPU_ERR("isp_apbm_clk clk_prepare failed, error=%d!\n", ret);
			return -EINVAL;
		}

		ret = clk_enable(hisijd->isp_apbm_clk);
		if (ret) {
			HISI_JPU_ERR("isp_apbm_clk clk_enable failed, error=%d!\n", ret);
			return -EINVAL;
		}
	}
#endif
	...
	if (hisijd->jpg_func_clk) {
		ret = clk_prepare(hisijd->jpg_func_clk);
		if (ret) {
			HISI_JPU_ERR("jpg_func_clk clk_prepare failed, error=%d!\n", ret);
			return -EINVAL;
		}

		ret = clk_enable(hisijd->jpg_func_clk);
		if (ret) {
			HISI_JPU_ERR("jpg_func_clk clk_enable failed, error=%d!\n", ret);
			return -EINVAL;
		}
	}
...
#ifdef CONFIG_NO_USE_INTERFACE
	/* note: regulator_disable have disable axim_clk apbm_clk */
	if (hisijd->isp_axim_clk) {
		clk_disable(hisijd->isp_axim_clk);
		clk_unprepare(hisijd->isp_axim_clk);
	}

	if (hisijd->isp_apbm_clk) {
		clk_disable(hisijd->isp_apbm_clk);
		clk_unprepare(hisijd->isp_apbm_clk);
	}
#endif

	if (hisijd->jpg_func_clk) {
		clk_disable(hisijd->jpg_func_clk);
		clk_unprepare(hisijd->jpg_func_clk);
	}
...
int hisijpu_enable_iommu(struct platform_device *pdev)
{
	...
	HISI_JPU_ERR("iommu_attach_device failed!\n");
	return -EINVAL;
	}*/
...
void hisijpu_disable_iommu(struct platform_device *pdev)
{
	...
	dev = &pdev->dev;
	if(g_hisijpu_domain) {
		iommu_detach_device(g_hisijpu_domain, dev);
		iommu_domain_free(g_hisijpu_domain);
	}
}
...
int hisi_jpu_register(struct hisi_jpu_data_type *hisijd)
{
	...
	hisijd->isp_axim_clk = devm_clk_get(&(hisijd->pdev->dev), hisijd->isp_axim_clk_name);
	if (IS_ERR(hisijd->isp_axim_clk)) {
		HISI_JPU_ERR("isp_axim_clk devm_clk_get error!");
		ret = -ENXIO;
		goto err_out;
	}

	hisijd->isp_apbm_clk = devm_clk_get(&(hisijd->pdev->dev), hisijd->isp_apbm_clk_name);
	if (IS_ERR(hisijd->isp_apbm_clk)) {
		HISI_JPU_ERR("isp_apbm_clk devm_clk_get error!");
		ret = -ENXIO;
		goto err_out;
	}
...
int hisi_jpu_on(struct hisi_jpu_data_type *hisijd)
{
	...
	hisijpu_regulator_enable(hisijd);
...
int hisi_jpu_off(struct hisi_jpu_data_type *hisijd)
{
	...
	hisijpu_regulator_disable(hisijd);
...
int hisi_jpu_lb_alloc(struct hisi_jpu_data_type *hisijd)
{
	...
	hisijd->lb_ion_handle = ion_alloc(hisijd->ion_client, lb_size,
		PAGE_SIZE * 8, ION_HEAP(ION_GRALLOC_HEAP_ID), 0);
#else
	hisijd->lb_ion_handle = ion_alloc(hisijd->ion_client, lb_size,
		PAGE_SIZE * 8, ION_HEAP(ION_SYSTEM_HEAP_ID), 0);
#endif
...
void hisi_jpu_lb_free(struct hisi_jpu_data_type *hisijd)
{
	...
	if (hisijd->lb_ion_handle) {
	#ifndef CONFIG_JPU_SMMU_DISABLE
...
int hisi_jpu_dec_set_cvdr(struct hisi_jpu_data_type *hisijd)
{
	...
	outp32(hisijd->jpu_cvdr_base + JPGDEC_CVDR_AXI_JPEG_NR_RD_CFG_1, 0x80000080);
	outp32(hisijd->jpu_cvdr_base + JPGDEC_CVDR_AXI_JPEG_NR_RD_CFG_2, 0x80000080);
...
int hisijpu_check_outbuffer(jpu_data_t *jpu_req)
{
	...
	*/
	...
	if ((jpu_req->stride_y < JPU_MIN_STRIDE)
		...
		|| (jpu_req->stride_y & (JPU_OUT_STRIDE_ALIGN - 1))) {
	...
	*/
	...
	if ((jpu_req->stride_c > JPU_MAX_STRIDE/2)
		|| (jpu_req->stride_c & (JPU_OUT_STRIDE_ALIGN - 1))) {
