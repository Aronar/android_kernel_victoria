...
err_device_create:
	if (hisijd->jpu_dev) {
		device_destroy(hisijd->jpu_class, MKDEV(hisijd->jpu_major, minor));
		hisijd->jpu_dev = NULL;
	}
...
{
	if ((!np) || (!hisijd)) {
		HISI_JPU_ERR("np %p or pdev %p NULL!\n", np, hisijd);
		return -ENXIO;
	}
...
static int hisi_jpu_probe(struct platform_device *pdev)
{
	...
	/* get isp_axim_clk_name resource */
	ret = of_property_read_string_index(np, "clock-names", 0, &(hisijd->isp_axim_clk_name));
	if (ret != 0) {
		HISI_JPU_ERR("failed to get isp_axim_clk_name resource! ret=%d.\n", ret);
		ret = -ENXIO;
		goto err_device_put;
	}

	/* get isp_axim_clk_name resource */
	ret = of_property_read_string_index(np, "clock-names", 1, &(hisijd->isp_apbm_clk_name));
	if (ret != 0) {
		HISI_JPU_ERR("failed to get isp_apbm_clk_name resource! ret=%d.\n", ret);
		ret = -ENXIO;
		goto err_device_put;
	}

	/* get isp_axim_clk_name resource */
	ret = of_property_read_string_index(np, "clock-names", 2, &(hisijd->jpg_func_clk_name));
	if (ret != 0) {
	...
	if (ret != 0) {
		HISI_JPU_ERR("failed to hisijpu_enable_iommu! ret=%d.\n", ret);
		return -ENXIO;
	}
