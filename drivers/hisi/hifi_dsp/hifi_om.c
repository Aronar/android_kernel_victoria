...
int hifi_get_dmesg(void __user *arg)
{
	...
	if (try_copy_from_user(&dump_info, arg, sizeof(struct misc_io_dump_buf_param))) {
	...
	ret = try_copy_to_user(dump_info_user_buf, s_dsp_dump_info[0].data_addr, len);
	if (ret != 0) {
		...
		size_t size, loff_t *data)
{
	char useage[] = "Useage:\necho \"test_case param1 param2 ...\" > dspfaultinject\n"
					...
					"DSP_slimbus_i2s_loop [str(on off)]\n"
