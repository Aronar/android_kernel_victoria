...
extern noinline int atfd_hisi_service_isp_smc(u64 funcid, u64 arg0, u64 arg1, u64 arg2);
...
static unsigned int get_boardid_version(void)
{
    ...
    if(root==NULL)
        return 1;
    if ((ret = of_property_read_u32_array(root, "hisi,boardid", boardid_version,4)) < 0 ) { /*lint !e838 */
        pr_err("Failed: board id of_property_read_u32.%d\n", ret);
        return 1;
    }
    if((boardid_version[0]==6)&&(boardid_version[3]==5)){
        return HISI_KIRIN970_CS;
    }
    return 0;
}
...
static void ispmmu_init(void)
{
    ...
    if(get_boardid_version() == HISI_KIRIN970_CS)
    {
        pr_info("Enter atf ispsmmu init +\n");
        atfd_hisi_service_isp_smc(ISP_FN_ISPSMMU_NS_INIT, pgt_addr, 0, 0);
        pr_info("Exit atf ispsmmu init -\n");
    }
    else
    {
...
static int isp_remap_rsc(struct hisi_isp_nsec *dev)
{
	...
	pr_info("[%s] isp_dma_va.0x%pK, dma.0x%llx\n", __func__, dev->isp_dma_va, dev->isp_dma);
...
static ssize_t bypass_power_up_and_down_show(struct device *pdev,
                   struct device_attribute *attr, char *buf)
{
    char *s = buf;

    s += snprintf(s, MAX_SIZE, "Bypass Power Up and Down : %s\n", (bypass_power_updn() ? "enable" : "disable"));

    return (s - buf);
}

static ssize_t bypass_power_up_and_down_store(struct device *pdev,
                ...
                size_t count)
{
    ...
    len = (size_t)p ? (size_t)(p - buf) : count;
    ...
    if ( !strncmp(buf, "enable", len) ) {
        ...
        printk("<Usage: > echo enable/disable > bypass_power_up_and_down\n");
    }
...
static DEVICE_ATTR(sec_nsec_isp, (S_IRUGO | S_IWUSR | S_IWGRP), sec_nsec_isp_show,
                    sec_nsec_isp_store);
static DEVICE_ATTR(bypass_power_up_and_down, (S_IRUGO | S_IWUSR | S_IWGRP), bypass_power_up_and_down_show,
                    bypass_power_up_and_down_store);/*lint !e846 !e778 !e866 !e84 !e514 */
...
int hisi_isp_nsec_probe(struct platform_device *pdev)
{
	...
	ret = device_create_file(nsec_isp_miscdev.this_device, &dev_attr_bypass_power_up_and_down);
	if (0 != ret) {
		pr_err("[%s] device_create_file failed, bypass_power_up_and_down ret.%d\n", __func__, ret);
		goto misc_de;
	}
