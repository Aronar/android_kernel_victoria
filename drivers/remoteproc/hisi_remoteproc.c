											...
											int offset, int avail)
{
    ...
    pr_info("%s: addr.0x%x, len.0x%x, kva.0x%pK, name.%s\n",
										...
										int offset, int avail)
{
	...
	pr_info("%s: exit, isp_share_para.0x%pK, va.0x%pK.\n", __func__, isp_share_para, va);
	return 0;
...
int set_plat_parameters(void)
{
    ...
    param->rdr_enable_type       |= A7ISP_RDR_USE_APCTRL; /*lint !e648*/
...
static int dis_reset_a7(struct rproc_boot_device *dev)
{
    ...
    RPROC_INFO("CRGPERI_A7_SEC_RSTDIS : 0x%pK = 0x%x\n", addr, value);
    if ((ret = dis_rststat_poll(addr, value)) != 0)
        RPROC_ERR("Failed : ISP_SEC_RSTDIS : 0x%pK = 0x%x, ret.0x%x\n", addr, value, ret);
...
#define ISP_2_PIPE_FLAG (1 << 20)
    ...
    if (boardid[0] == 6 && boardid[1] == 0 && boardid[2] == 9) {
        if (boardid[3] == 5 || boardid[3] == 6) {
...
static struct hisi_rproc_data *hisi_rproc_data_dtget(struct device *pdev)
{
    ...
    if (!use_sec_isp() && !use_nonsec_isp()) {
        ...
        RPROC_INFO("isp_subsys_ip.0x%pK \n", dev->isp_subsys_ip);
    }
